...
export function GET(request: NextRequest) {
  ...
  return NextResponse.json(listBudgetsWithSpent(month));
}
...
export async function POST(request: NextRequest) {
  ...
  upsertBudget(parsed.data.categoryId, parsed.data.month, parsed.data.limitAmount);
  const all = listBudgetsWithSpent(parsed.data.month);
